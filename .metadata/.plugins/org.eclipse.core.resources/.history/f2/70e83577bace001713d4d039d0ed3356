...
public class Runner 
{
	...
	public static String[] getParamTypes(String args[])
	{
		...
		else if(!args[0].equalsIgnoreCase("all")&&(!args[0].contains(":"))&&!args[0].contains(",")) {
				params[0] = args[0];
				params[1] = null;
		}
		else if(args[0].contains(",")&&!args[0].equalsIgnoreCase("all")&&(!args[0].contains(":"))) {
	...
	//	screenshotLocation = configproperties.getProperty("MOUNTPATH").trim()+"/"+projectName+"/"+projecttype+"/"+customer+"/"+sprintName+"/"+testtype+"/Reports/Screenshot";
		resultLocation = configproperties.getProperty("MOUNTPATH").trim()+"/"+projectName+"/Reports"+"/"+sprintName;
		
		elements = properties(configproperties.getProperty("CONFIGELEMENTLOCATION"));
		reporter = Template.createReporter(reporter, resultLocation,sprintName);
